...
def main():
    ...
    if not ds_dir.exists():
        ...
        with open(ds_dir / "train.pkl", "rb") as f:
            ...
            loader_train = DataLoader(loader_train.dataset, cfg.batch_size, shuffle=True)
        with open(ds_dir / "valid.pkl", "rb") as f:
            loader_valid: DataLoader = pkl.load(f)
            loader_valid = DataLoader(loader_valid.dataset, cfg.batch_size, shuffle=True)
    ...
    for epoch in tqdm(range(cfg.num_epochs), desc="epoch"):
        ...
        for batch in tqdm(loader_train, desc="batch", leave=False):
            ...
            loss.backward()
            opt.step()
        total_loss /= (num_train // cfg.batch_size) + 1
